...
NOTES_HAPPYBIRTHDAY = [
    ('C4', 350), ('C4', 150), ('D4', 500), ('C4', 500), ('F4', 500), ('E4', 900), ('REST', 100),
...
def api_alarms(req):
    ...
    if req.method == "GET":
        ...
        try:
            ...
            song = data.get("song", "NOTES_STAR")  # 預設值
            new_alarm = {
...
def test(req):
    global is_ringing
    is_ringing = True
    song = NOTES_STAR  # 預設值（或你可以改成測試指定的曲名）
    asyncio.create_task(ring_task(song))
    ...
    while True:
        ...
        for i, a in enumerate(alarms):
            ...
            if alarm_time == now and i not in last_triggered:
                ...
                song_data = globals().get(song_name, NOTES_STAR) # 把字串轉成對應變數，例如 "NOTES_STAR" → NOTES_STAR
