...
本專案實作一個 ESP32-S2 控制的智慧鬧鐘系統，結合 OLED 顯示、網頁控制、非同步播放音樂、MQTT資料傳送等功能，透過前後端協作實現遠端鬧鐘管理。
...
- 📺 OLED 實時顯示時間與鬧鐘
- 🧠 非同步音樂播放（uasyncio）
- ☁️ MQTT 物聯網整合**：實時上傳時間、鬧鐘狀態與設定變更至 Broker (EMQX)
...
- OLED 實時更新時間與鬧鐘狀態
- 鬧鐘觸發後會播放對應音樂
- 透過MQTT通訊協定傳輸資料(當前時間、使用者設定鬧鐘、鈴聲狀態)至Broker
